...
?>
<article class="sk-template-card h-100">
    <a class="sk-template-card__thumb" href="<?= e($href) ?>"
       style="--sk-thumb-a:<?= eattr($t['color_primary'] ?? '#C8102E') ?>;--sk-thumb-b:<?= eattr($t['color_secondary'] ?? '#F0A02A') ?>">
        <?php if ($thumb !== ''): ?>
            <img src="<?= e(url($thumb)) ?>" alt="<?= eattr($t['name']) ?>" loading="lazy" decoding="async" width="400" height="560">
        <?php else: ?>
            <span class="sk-template-card__initial" aria-hidden="true"><?= e(mb_substr((string) $t['name'], 0, 1)) ?></span>
        <?php endif; ?>
        <span class="sk-template-card__flags">
            <?php if ((int) ($t['is_premium'] ?? 0) === 1): ?>
                <span class="sk-badge sk-badge--premium"><i class="bi bi-star-fill"></i> <?= e(__('templates.premium')) ?></span>
            <?php endif; ?>
            <?php if ((int) ($t['has_animation'] ?? 0) === 1): ?>
                <span class="sk-badge sk-badge--animated"><i class="bi bi-magic"></i> <?= e(__('templates.animated')) ?></span>
            <?php endif; ?>
            <?php if ((int) ($t['page_count'] ?? 1) > 1): ?>
                <span class="sk-badge sk-badge--pages"><?= (int) $t['page_count'] ?> <?= e(__('templates.pages')) ?></span>
            <?php endif; ?>
        </span>
    </a>
    <div class="sk-template-card__body">
        <h3 class="sk-template-card__name"><a href="<?= e($href) ?>"><?= e($t['name']) ?></a></h3>
        <p class="sk-template-card__meta">
